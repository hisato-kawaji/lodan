...
- **`!<cmd>`**: 行頭の `!` に続くコマンドをシェル（`sh -c`、cwd、Bash ツールと同じ timeout で、時間切れなら子を殺す）で直接実行して出力を表示し、**次のユーザ発話の文脈に添える**（`!git status` してから「これを整理して」など）。打ったのは利用者自身なので**承認ゲートもサンドボックスも deny ルールも通さない** — モデルが書いたコマンドではないから。`-p`（ヘッドレス）では効かず、そのままプロンプトになる。出力は 16 KiB まで（切り詰めた後に `(exit N)` を付ける）。コマンドとして扱うのは**先頭の語がシェルの組み込みか PATH 上の実行ファイルのとき**だけで、`!important` や `!!! 急ぎで` のような発話はそのままモデルへ渡る。`!` で始まる発話を確実にモデルへ渡したいときは `\!…`
