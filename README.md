...
- **空応答の促し**(`[agent] empty_reply_nudge`、`--empty-reply-nudge[=<bool>]`、既定 **true**): 本文もツール呼び出しも無い応答(thinking モデルが思考だけ書いて閉じたもの。qwen3.5:9b の実測で 20 実行中 13)が来たら、1 回だけ「考えた内容を踏まえて答えを書くか次のツールを呼べ」と促して続ける。それでも空なら、そのまま終える(`-p` は終了コード `1` で、`error` に「思考は N 文字あったが答えを書かなかった」と出る)。思考を返さないモデルでは発火しない(#111)
