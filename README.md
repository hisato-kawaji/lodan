...
- `workspace-write` でも、作業ディレクトリの中の次の場所は書けません: `.lodan/`、`.env`、`.mcp.json`、`.git/hooks/`、`.git/config`。ここに書けると、サンドボックスの中のコマンドが「次にサンドボックスの外で動くもの」（lodan の設定や hooks、MCP サーバ、git の hooks）を仕込めてしまうためです。`git commit` などは通常どおり動きます。macOS ではまだ存在しないファイルの作成も止まりますが、**Linux（bwrap）で守れるのは起動時に既に存在するパスだけ**です。
...
- サンドボックスに止められたらしい失敗（`Operation not permitted` など）には、結果に `[sandbox]` の注記が付きます。モデルが同じコマンドを繰り返したり抜け道を探したりせず、ユーザーに伝えるようにするためです。
