...
| `think-none` / `think-low` | `temp` + `--reasoning-effort none` / `low` | 推論の深さ (#78)。**`temp` (サーバ既定の effort) と比べる**。語彙はサーバ次第 (Ollama は `none` で thinking off、kimi / gpt-oss は `low`)。既定の `CONFIGS` には含めない |
...
- **秒数は単調時計を見る**。`results.jsonl` の `secs` は壁時計なので、実行中に
  マシンがスリープすると実作業と無関係に膨らむ (実測で 372s の作業が 35,793s と
  記録された)。`summarize.py` は `metrics.active_ms` (lodan の `Instant` 由来) を
  優先し、乖離した実行を末尾に列挙する
- **ハーネスを途中で kill すると同じ key の行が二重に出ることがある**
  (生き残った子シェルと再開後の実行が両方書く)。`summarize.py` は後勝ちで
  畳むが、行を直接数えるときは注意する

## workspace trust

`ladder.sh` は作業ディレクトリに `.lodan/config.toml` (タイムアウト設定) を書いてから lodan を起動する。
lodan は未信頼のディレクトリのプロジェクト設定を読まないので、ハーネスは `--trust` を付けて起動している。
