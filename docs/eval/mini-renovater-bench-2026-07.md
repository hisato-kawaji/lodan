...
## 対策後の再試験(v2, 2026-07-24)

考察に基づき lodan へ 3 対策(温度制御 / 壊れツールコール再要求 / 重複 read-only 抑止 — PR #62)を実装し、temperature 0.2 で同条件(720s)再試験した。

| | qwen3.5:9b before | qwen3.5:9b v2 | gemma4:12b before | gemma4:12b v2 |
|---|---|---|---|---|
| 到達 | 1/5 | 0/5 | 0/5 | 0/5 |
| no-tools 即死 / SSE 断 | 2 ステージ | **0** | 4 ステージ | **0**(即死 2 は非破綻の早期終了) |
| 実作業中の TIMEOUT | 3 | 4 | 2 | 3 |
| ollama パース WARN(期間中) | 11 件 | — | — | **2 件** |

**解釈**: スコアは改善しなかったが、失敗モードが「整形破綻・ループによる即死」から「正しい方向の作業中の時間切れ」へ一本化された(対策 B/D の発火は 0 回 = 温度 0.2 の時点で破綻自体がほぼ消えた)。ボトルネックは能力の破綻から純粋なスループット(9B/12B on M3 Air の物理限界)へ移った — 前回は無意味だったタイムアウト延長が、今回は意味を持つ状態になった。

## 延長 v3 と finish_nudge v4(2026-07-25)

v2 の帰結「延長すれば通るかもしれない」を v3(2400s)で検証 → **両モデル 0/5**。原因は時間でなく:

- qwen S1: 33 分・11 回の自己編集の末、仕様中核の「JSON 書き込み」が実装から**脱落**(dict を print するだけ)→ 全ステージがカスケード死
- gemma S3/S5: **plan-only 停止**(「まず read します」と宣言だけして 49-66 秒でツール 0 回終了)

この 2 モードに対し **finish_nudge**(終了前自己検証ナッジ、lodan PR #64、既定 off の opt-in)を実装し v4 で再検証:

| | v3 (2400s) | v4 (2400s + nudge) |
|---|---|---|
| qwen3.5:9b | 0/5 | 0/5(ただし S1 は **JSON を書くようになり** stack 検出のみで失敗 — 要件脱落は解消) |
| gemma4:12b | 0/5 | 0/5(S1 は JSON 書き込み済み・stack 空で失敗。S5 はナッジ 1 回でも plan-only 継続) |

ナッジ発火: qwen ×5 / gemma ×2。**失敗はステージ丸ごとの崩壊から「サブ要件 1 個の不足」へ前進した**が、スコアには届かず。

**最終結論**: 温度 0.2 + 整形回復 + 重複抑止 + 終了前ナッジまで積んでも、9B/12B 級は多段累積のプロダクト組み上げに届かない(単発小タスクとの能力段差は対策では埋まらない)。この用途は API 級(fugu 5/5)または大型ローカル(30B 級、本機では未測定)の領分。
